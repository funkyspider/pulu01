...
public class ProcessingPersistenceService : IProcessingPersistenceService
{
    ...
    public async Task<HashSet<string>> LoadProcessedRecordsAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            if (File.Exists(_configuration.SuccessLogPath))
            {
                ...
                {
                    var records = JsonSerializer.Deserialize<List<ProcessedRecord>>(json, HttpApiClientService.SharedJsonOptions) ?? new List<ProcessedRecord>();
    ...
    private async Task FlushSuccessfulRecordsAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (File.Exists(_configuration.SuccessLogPath))
            {
                ...
                {
                    existingRecords = JsonSerializer.Deserialize<List<ProcessedRecord>>(existingJson, HttpApiClientService.SharedJsonOptions) ?? new List<ProcessedRecord>();
                }
            }
            ...
            var json = JsonSerializer.Serialize(existingRecords, new JsonSerializerOptions { WriteIndented = true, TypeInfoResolver = AppJsonContext.Default, PropertyNameCaseInsensitive = true });
    ...
    private async Task WriteFailedRecordImmediately(ProcessingResult result, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var json = JsonSerializer.Serialize(failedRecord, HttpApiClientService.SharedJsonOptions);
